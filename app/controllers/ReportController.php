...
class ReportController extends ControllerBase
{
    protected function _generateReport($user, $startDate, $endDate)
    {
        ...
        $results = $this->modelsManager->executeQuery('SELECT MAX( logged_hours ) AS max_logged_hours, MAX ( num_tasks_worked )  AS max_num_tasks_worked FROM ReportDaily WHERE user_id = "' . $user->id . '" AND date >= "' . $startDate . '" AND date <= "' . $endDate . '"');
        ...
        foreach ($results AS $result) {
            ...
            $max_num_tasks_worked = $result->max_num_tasks_worked;
        }
        ...
        foreach ($dbReports AS $dbReport) {
            ...
            $temp['num_tasks_worked_value'] = round(($dbReport->num_tasks_worked / $max_num_tasks_worked), 4) * 100;
            $temp['num_tasks_worked_label'] = $dbReport->num_tasks_worked;
